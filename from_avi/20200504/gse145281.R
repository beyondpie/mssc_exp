...
library(uwot)
library(parallel)
exp.file <- list.files(path = "~/liulab_home/data/single_cell/GSE145281/",
                       pattern = "*raw.txt.gz", full.names = T) %>%
...
gse145281 <- CreateSeuratObject(counts = t(GSE145281.exp.mat),
                                project = "GSE145281", min.cells = 5) %>%
...
p1 <- DimPlot(object = gse145281,
              reduction = "harmony", pt.size = .1, group.by = "patient")
...
DimPlot(gse145281, reduction = "umap",
        group.by = "patient", pt.size = .1, split.by = "patient") %>%
  ggsave(".figs/gse145281/gse145281.harmony.umap.patient.pdf", plot = .)

DimPlot(gse145281, reduction = "umap", group.by = "patient",
...
## Implementation of psuedo-bulk method from
## https://www.biorxiv.org/content/10.1101/2020.04.01.019851v1
...
deseq.out <- DESeq2DETest(data.use = psuedo.bulk,
