...
get_fitted_mu <- function(y, vec_of_cond,
  ...
  invisible(list(
    ...
    r_cond = c(y_control_fit$r, y_case_fit$r)
    ## mu_ind = c(mu_control_ind, mu_case_ind),
    ## r_ind = c(r_control_ind, r_case_ind)
  ))
}
```
...
y <- vapply(seq_len(num_of_cell_per_ind * num_of_ind),
  FUN = function(i) {
    set.seed(1)
    logmu <- rnorm(1, mean = mug + mucond[vec_of_cond[i]],
                   sd = sqrt(kappa2g))
    set.seed(1)
    rnbinom(
      n = 1,
