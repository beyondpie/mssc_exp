---
title: "Check MSSC: modeling individual effect in mean"
author: Songpeng Zu
...
# STAN model setting: with individual mean

## load stan model
```{r mssc,cache=TRUE}
sbc_gwnb_model <- cmdstan_model(here::here(
  "src", "dirty_stan",
  "gwnb_simu_from_prior.stan"
), compile = T)

## show the stan content
## sbc_gwnb_model$print()
```

## set data and initial values

```{r sample_Data_from_prior, cache= TRUE}
vec_of_cond <- c(
...
init_params <- list(
  ...
  MuIndRaw = rnorm(N, 0.0, sqrt(kappa2g)),
...
```{r mydata, cache=TRUE, echo=FALSE}
...
```{r sbc_gwnb_vi_randinit, cache=TRUE}
