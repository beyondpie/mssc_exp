...
// Given a specific indivdiual, we model individual effects across the genes firstly.
...
model {
	...
	// mu_cond ~ N(0.0, sqrt(varofcond))
	to_vector(raw_mu_cond) ~ std_normal();
	// mu_ind ~ N(0.0, sqrt(varofind))
